...
export const PreviewSettingsPopover = ({ projectId, initialValues, onSave }: PreviewSettingsPopoverProps) => {
    ...
    const updateSettings = useUpdateProjectSettings(); // easier to use this hook and maintain
    ...
    const handleOpenChange = (isOpen: boolean) => {
        // reset the form when the popover is opened
        if (isOpen) {
            form.reset({
                installCommand: initialValues?.installCommand ?? "",
                devCommand: initialValues?.devCommand ?? "",
            ...
            <PopoverContent className="w-80" align="end">
                ...
                <form.Field name="devCommand">
                    {(field) => (
                        <Field>
                            ...
                            />
                            <FieldDescription>Command to run the dev server</FieldDescription>
                        </Field>
                    )}
                </form.Field>
                ...
                <form.Subscribe selector={(state) => [state.canSubmit, state.isSubmitting]}>
                    {([canSubmit, isSubmitting]) => (
                        ...
                        // disable the button if we can't submit or if we are submitting
                        >
