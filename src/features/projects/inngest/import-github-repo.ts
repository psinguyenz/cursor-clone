...
export const importGithubRepo = inngest.createFunction(
    ...
    async ({ event, step }) => {
        ...
        await step.run("cleanup-existing-files", async () => {
            const existingFiles = await convex.query(api.system.getProjectFiles, {
                internalKey,
                projectId,
            });
        });
        ...
        // Sort folders by depth so paretns are created before children
        // Input: [{ path: "src/components"}, {path: "src"} { path: "src/components/ui"}]
        // Output: [{ path: "src"}, { path: "src/components"}, { path: "src/components/ui"}]
        // Octokit just return us a bunch of files and structures but we have to create them in the way that we create their parent first
        ...
        const folderIdMap = await step.run("create-folders", async () => {
            ...
            for (const folder of folders) {
                ...
                const name = pathParts.pop()!; // seperating the name from the path
