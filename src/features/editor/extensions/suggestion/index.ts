...
// - create(): Retruns the initial value when the editor loads
...
const generatePayload = (view: EditorView, fileName: string) => {
    ...
    for (let i=1; i<= linesToFetch; i++) {
        nextLines.push(view.state.doc.line(currentLine.number + 1).text);
    }
...
const renderPlugin = ViewPlugin.fromClass(
    class {
        ...
        update(update: ViewUpdate) {
            ...
            if (shouldRebuild) {
                this.decorations = this.build(update.view); // display something new to the user
            }
        }
        ...
        build(view: EditorView) {   
            ...
            return Decoration.set([
                Decoration.widget({
                    widget: new SuggestionWidget(suggestion), // show this to the user
...
export const suggestion = (fileName: string) => [
    suggestionState, // Our state storage (current suggestion)
