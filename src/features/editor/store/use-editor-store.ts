...
export const useEditorStore = create<EditorStore>()((set, get) => ({
    ...
    openFile: (projectId, fileId, { pinned }) => {
        ...
        console.log(`Open File: ${fileId} | Pinned: ${pinned} | IsOpen: ${isOpen} | PreviewID: ${previewTabId}`);
    ...
    closeTab: (projectId, fileId) => {
        ...
        if (tabIndex === -1) return; // It's already closed
        ...
        if (activeTabId === fileId) {
            if (newTabs.length === 0) {
                ...
                newActiveTabId = newTabs[newTabs.length - 1]; // the last one we opened
            } else {
