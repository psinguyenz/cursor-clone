...
export const ConversationSidebar = ({ projectId }: ConversationSidebarProps) => {
    ...
    // if the user select, we gonna compute selectedConversationId
    // If the user just reset the page, we'll fall back to pick project latest conversation
    // If that also didn't exist then we go back to null
    ...
    const handleSubmit = async (message: PromptInputMessage) => {
        ...
        if (!conversationId) {
            conversationId = await handleCreateConversation(); // for new user
            if (!conversationId) {
                return;
            }
        }
            ...
            <div className="flex flex-col h-full bg-sidebar">
                <div className="h-8.75 flex items-center justify-between border-b">
                    ...
                    <div className="flex items-center px-1 gap-1">
                        <Button size="icon-xs" variant="highlight" onClick={() => setPastConversationOpen(true)}>
                ...
                <Conversation className="flex-1">
                    <ConversationContent>
                        {conversationMessages?.map((message, messageIndex) => (
                            ...
                            >
                                <MessageContent>
                                    {/* the user can either send a new message or cancel the current message */}
